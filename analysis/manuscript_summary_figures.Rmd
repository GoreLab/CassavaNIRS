...
germ.by.trial %>% arrange(-sum_representation) %>% View()
...
dmc.violin.boxplot <- plots.aggregated %>% left_join(tuk.groups, by = "studyName") %>% 
  full_join(namekey) %>% 
  ggplot(aes(x = Trial, y = dry.matter.content.percentage.CO_334.0000092, 
             fill = Trial
             )) + geom_violin()+
  geom_boxplot(position = "identity", width = .2) + 
  theme_bw() + 
  geom_text(aes(label = groups, y = (.6 + Max)), vjust = 0) +
  labs(x = element_blank(), #title = "Plot mean dry matter content by trial", 
